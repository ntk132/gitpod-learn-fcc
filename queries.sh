...
echo "$($PSQL "SELECT name FROM teams AS t INNER JOIN games AS g ON t.team_id = g.winner_id OR t.team_id = g.opponent_id WHERE round='Eighth-Final' AND year=2014 ORDER BY name;")"
